...
core/
  api/
    ...
    models/
      ...
      cashflow.py         # CashflowItem, CashflowResponse — GET /pool/{poolAddress}/transactions/cashflows
                          #   params: investorAddress, type (Deposit|Withdrawal), limit, page
  ui/
    base_page.py
...
- **TRX-тесты: ссылка на транзакцию** — в каждом тесте с on-chain транзакцией вызывать `attach_tx_link(tx_hash)` в теле теста. Правило: всегда, независимо от результата теста. Arbitrum → Arbiscan, Ethereum → Etherscan. Функция `attach_tx_link(tx_hash, chain="arbitrum")` определена в `test_deposit_trx.py` (TODO: вынести в `core/ui/helpers/` если понадобится в других модулях).
